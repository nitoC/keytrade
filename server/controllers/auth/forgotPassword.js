...
const person=require('../../models/models')
 ...
 const forgotPassword=async (req,res)=>{
     ...
     let {email}=req.body
        try{
            const user=await person.find({email},(error,response)=>{
                if(error){
           ...
           userUpdate=await person.findOneAndUpdate({email},{resetToken:token,expireToken:Date.now()+900000},(err,respose)=>{
              if(err){
